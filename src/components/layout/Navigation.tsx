...
import { NAV_LINKS, PRICE_LINKS } from "../../constants/navigation";
...
const Navigation = () => {
  ...
  return (
    ...
    >
      <div className="container-site">
        <div className="flex items-center justify-between h-20 lg:h-24">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-white font-semibold tracking-widest text-sm lg:text-base uppercase">
              BAMBUAM
            </span>
          </Link>
      ...
      <div 
        className={cn(
          "lg:hidden fixed inset-0 top-20 bg-bambuam-dark/98 backdrop-blur-lg transition-all duration-300",
        ...
        <div className="container-site py-8 space-y-2">
          {NAV_LINKS.map((link, index) => (
            <Link
              ...
              to={link.path}
              className={cn(
                "block py-4 text-lg font-medium border-b border-bambuam-border transition-all duration-300",
                isActive(link.path) 
                  ? "text-bambuam-sage" 
            ...
            <div className="mt-3 space-y-2 pl-4">
              {PRICE_LINKS.map((link) => (
                <Link 
                  key={link.path}
                  to={link.path} 
                  className="block py-2 text-bambuam-text-secondary hover:text-white transition-colors"
                >
                  {link.name}
                </Link>
              ))}
